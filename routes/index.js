    ...
    if (!bool) {
      ...
      if (code != undefined) {
        ...
        if (!devices[0].product | !codecId) {
          ...
          getJwtToken().then(t => {
            Promise.all([
              getAccessTokenFromGuest(t),
              getRoomAnalytics(token, codecId)
            ]).then(values => {
              getUserDetails(values[0]).then(userDetails => {
                res.render("index", {
                  title: "Controller",
                  codecId: codecId,
                  devices: devices,
                  ...
                  guestAccessToken: values[0],
                  userSip: userDetails.emails[0]
                });
              });
            });
          });
...
router.get("/setPosition", async function(req, res, next) {
  ...
  if (position == "error") {
    res.status(500).json({
      error:
        "something is wrong try to refresh the token or check device id or check device id"
    });
  } else {
    var options = {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        arguments: {
          CameraId: 1,
          Pan:
            position.Zoom > 3500 && (position.Pan > 100 || position.Pan < -100)
              ? pan * 5
              : pan,
...
function getPosition(token, deviceId) {
  return new Promise(resolve => {
    var options = {
      method: "GET",
      url:
        "https://webexapis.com/v1/xapi/status?deviceId=" +
        deviceId +
        "&name=cameras.*",
      headers: {
        Authorization: "Bearer " + token
...
function getRoomAnalytics(token, deviceId) {
  ...
  return new Promise(resolve => {
    var options = {
      method: "GET",
      url:
        "https://webexapis.com/v1/xapi/status?deviceId=" + deviceId + "&name=*",
      headers: {
        Authorization: "Bearer " + token
      }
    };
      ...
      try {
        ...
        var roomAnalyticsData = {
          ...
          peripherals: JSON.parse(
            response.body
          ).result.Peripherals.ConnectedDevice.filter(word =>
            word.Name.toLowerCase().includes("navigator")
          )
        };
